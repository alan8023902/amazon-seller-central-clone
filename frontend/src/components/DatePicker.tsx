...
const DatePicker: React.FC<DatePickerProps> = ({ 
  ...
  return (
    <div className="relative">
      <div 
        className="flex items-center border border-[#888] bg-white h-[25px] px-1 cursor-pointer w-24"
        onClick={() => setIsOpen(!isOpen)}
      >
        <Calendar size={12} className="text-[#0F1111] mr-1 flex-shrink-0" />
        <input 
          type="text" 
          className="border-none outline-none w-16 text-[10px] text-[#0F1111] cursor-pointer bg-transparent"
          value={selectedDate}
          readOnly
        />
      </div>
